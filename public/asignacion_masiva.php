 ...
 * Versión 4.0 Final - Responsive, Transaccional y con Redirección Automática
 */
require_once '../core/db.php';
require_once '../core/session.php';

// 1. SEGURIDAD RBAC
// Bloqueamos acceso a Auditor, solo permitido para Admin, Recursos, Soporte
...
$msg = "";
$step = 1; // Control de fases: 1=Subir, 2=Validar
$preview_data = [];
$csv_errors = false;
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['upload_csv'])) {
    if (isset($_FILES['csv_file']) && $_FILES['csv_file']['error'] == 0) {
        ...
        if (strtolower($ext) !== 'csv') {
            $msg = "<div class='alert error'>❌ Formato incorrecto. Solo se permiten archivos .csv</div>";
        } else {
            ...
            while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
                ...
                // Saltar encabezados si se detectan
                if ($row_count === 1 && (stripos($data[0], 'PLACA') !== false)) continue;
                // Limitar a 100 registros por seguridad de rendimiento
                ...
                // Validar contra DB (Equipo existe y está en Alta)
                ...
                if (!$equipo) { 
                    ...
                    $status = 'duplicated'; 
                    $note = 'Placa repetida en archivo'; 
                    $csv_errors = true; 
                } else { 
            ...
            if (count($preview_data) > 0) {
                $step = 2; // Avanzar a vista previa
            } else {
                $msg = "<div class='alert error'>⚠️ El archivo está vacío o no tiene el formato correcto.</div>";
            }
        }
    }
}

// FASE 2: GUARDADO TRANSACCIONAL Y REDIRECCIÓN
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['confirm_save'])) {
    try {
        ...
        $serials_procesados = []; // Array para recolectar los seriales exitosos
        ...
        foreach ($items as $item) {
            ...
            $stmt->execute([
                ...
                (isset($_POST['check_dlo']) ? 1 : 0), 
                (isset($_POST['check_antivirus']) ? 1 : 0)
            ]);
        ...
        // **REDIRECCIÓN AUTOMÁTICA**: Si hubo éxito, ir directo al acta
        if (count($serials_procesados) > 0) {
            ...
            exit; // Detener ejecución para asegurar el redirect
        } else {
        ...
        $msg = "<div class='alert error'>❌ Error del Sistema: " . $e->getMessage() . "</div>";
        $step = 2;
    }
}
?>
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0"> <title>Asignación Masiva | URTRACK</title>
    <style>
        ...
        .header a { text-decoration: none; color: #64748b; font-weight: 500; }
        ...
        /* Tabla Responsive */
        .table-container { 
            width: 100%; overflow-x: auto; border: 1px solid #e2e8f0; 
            margin-top: 20px; border-radius: 6px; 
        }
        .preview-table { width: 100%; border-collapse: collapse; min-width: 600px; }
        .preview-table th { background: #f1f5f9; padding: 12px; text-align: left; font-size: 0.9rem; color: #475569; }
        ...
        input[type="text"], select, input[type="file"] { 
            width: 100%; padding: 12px; border: 1px solid #cbd5e1; border-radius: 6px; box-sizing: border-box; 
        }
        ...
        .btn-primary { 
            ...
            opacity: 0.5; cursor: not-allowed; transition: all 0.3s; 
        }
        .btn-primary:not(:disabled) { opacity: 1; cursor: pointer; box-shadow: 0 4px 6px -1px rgba(79, 70, 229, 0.2); }
        .btn-cancel { 
            background: #64748b; color: white; padding: 15px; border-radius: 6px; 
            text-decoration: none; text-align: center; display: flex; 
            align-items: center; justify-content: center; font-weight: bold; width: 100%;
        }
        ...
        @media (max-width: 768px) {
            ...
            .btn-primary { order: 1; } /* El botón confirmar primero en móviles */
            .btn-cancel { order: 2; }
        }
    </style>
</head>
...
<div class="card">
    ...
    <?php if ($step === 1): ?>
        <div style="text-align:center; padding: 40px 20px; border: 2px dashed #cbd5e1; border-radius: 12px; background: #fafafa;">
            ...
            <code style="background:#e2e8f0; padding:2px 5px; border-radius:4px; font-size:0.9rem;">PLACA_UR, HOSTNAME, [INFO_ADIC1], [INFO_ADIC2]</code></p>
    ...
    <?php if ($step === 2): ?>
        <form method="POST">
            ...
            <div class="action-buttons">
                ...
                <div style="flex:2;">
                    <?php if ($validos > 0): ?>
                        <button type="submit" name="confirm_save" id="btnSubmit" class="btn-primary" disabled>
                            CONFIRMAR (<?= $validos ?> Equipos) 🔒
                        </button>
                    <?php else: ?>
                        <div class="alert error" style="margin:0;">No hay equipos válidos</div>
                    <?php endif; ?>
                </div>
            </div>
        </form>
